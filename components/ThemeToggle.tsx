...
function useTheme() {
  ...
  useEffect(() => {
    ...
    // 监听其他document对localStorage的修改， 比如直接在浏览器调试页面的Application修改localStorage的值
...
export function ThemeToggle({ panelClassName = "mt-4" }) {
  ...
  return (
    <Listbox value={setting} onChange={setSetting}>
      ...
      <Listbox.Options
        className={clsx(
          "absolute z-50 right-0 top-full bg-white rounded-lg shadow-lg overflow-hidden ring-1 ring-slate-900/10 w-36 py-1 text-sm text-slate-700 font-semibold dark:bg-slate-800 dark:ring-0 dark:highlight-white/5 dark:text-slate-300",
          panelClassName
        )}
      >
